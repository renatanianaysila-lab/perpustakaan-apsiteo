...
<div class="flex min-h-screen">
    ...
    <main class="ml-64 flex-1 p-8">
        ...
        <div class="flex items-start justify-between mb-6">
            <div>
                <h2 class="text-2xl font-bold text-slate-800">Kelola Data Buku</h2>
                <p class="text-sm text-slate-500">Daftar koleksi buku perpustakaan pusat yang tersedia saat ini.</p>
            </div>
        ...
        <div class="bg-white rounded-xl p-4 shadow-sm border border-slate-100 mb-6">
            <form method="GET" action="{{ route('admin.buku.index') }}" class="flex items-center gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="q" value="{{ $keyword ?? '' }}"
                           placeholder="Cari kode, judul, atau pengarang..."
                ...
                <button type="submit" class="px-5 py-2.5 bg-blue-900 text-white text-sm font-medium rounded-lg hover:bg-blue-950 transition">
                    Cari
                </button>
                @if(!empty($keyword))
                    <a href="{{ route('admin.buku.index') }}" class="px-4 py-2.5 text-sm text-slate-500 hover:text-slate-700">Reset</a>
                @endif
            </form>
        </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-blue-950 text-white text-left">
                            <th class="px-4 py-3 font-medium">Kode Buku</th>
                            <th class="px-4 py-3 font-medium">Judul</th>
                            <th class="px-4 py-3 font-medium">Pengarang</th>
                            <th class="px-4 py-3 font-medium">Penerbit</th>
                    ...
                    <tbody>
                        @forelse($buku as $item)
                            <tr class="border-t border-slate-100 hover:bg-slate-50">
                                <td class="px-4 py-4 text-slate-500 whitespace-nowrap">{{ $item->id_buku }}</td>
                                <td class="px-4 py-4 font-semibold text-slate-800">{{ $item->judul_buku }}</td>
                                <td class="px-4 py-4 text-slate-600">{{ $item->pengarang }}</td>
                                <td class="px-4 py-4 text-slate-600">{{ $item->penerbit }}</td>
                                <td class="px-4 py-4 text-slate-600">{{ $item->tahun_terbit }}</td>
                                <td class="px-4 py-4">
                                    <span class="px-2.5 py-1 text-xs font-medium bg-cyan-50 text-cyan-700 rounded-full">
                                        {{ $item->kategori->nama_kategori ?? '-' }}
                                    </span>
                                </td>
                                <td class="px-4 py-4 text-center font-bold {{ $item->stok_tersedia == 0 ? 'text-red-500' : 'text-slate-800' }}">
                                    {{ $item->stok_tersedia }}
                                </td>
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-center gap-3">
                                        ...
                                        <form method="POST" action="{{ route('admin.buku.destroy', $item->id_buku) }}"
                                              onsubmit="return confirm('Yakin ingin menghapus buku &quot;{{ $item->judul_buku }}&quot;?')">
                                            @csrf
                                            @method('DELETE')
