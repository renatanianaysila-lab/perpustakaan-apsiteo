...
<div class="flex min-h-screen">
    ...
    <main class="ml-64 flex-1 p-8">
        ...
        <div class="mb-6">
            ...
            <h2 class="text-2xl font-bold text-slate-800 mt-2">Edit Buku</h2>
            <p class="text-sm text-slate-500">Perbarui data buku <span class="font-medium">{{ $buku->judul_buku }}</span>.</p>
        </div>
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-slate-100 max-w-3xl">
            <form method="POST" action="{{ route('admin.buku.update', $buku->id_buku) }}" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div>
                        ...
                        <select name="id_rak" class="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-200">
                            @foreach($rak as $r)
                                <option value="{{ $r->id_rak }}" @selected(old('id_rak', $buku->id_rak) == $r->id_rak)>{{ $r->nama_rak }} ({{ $r->lokasi_rak }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Stok Total</label>
                        <input type="number" name="stok_buku" value="{{ old('stok_buku', $buku->stok_buku) }}" min="0"
                               class="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-200">
                        <p class="text-xs text-slate-400 mt-1">Tersedia saat ini: {{ $buku->stok_tersedia }}</p>
                    </div>
                </div>
